      ...
      <header className="flex items-center justify-between w-full h-20 pr-10 bg-white pl-36 shadow-lg shadow-[#5B5B5B21]">
        ...
        <div className="relative">
          <button onClick={handleShowWind} className="flex items-center h-9">
            ...
            <span className="ml-2.5 font-semibold text-sm">Bangura123</span>
            <i className={`ml-1 fa-solid fa-chevron-${showWind ? "up" : "down"} text-darkblue/50`}></i>
          </button>
          {showWind && (
            <div className="absolute prfwindow flex flex-col bg-[#F3F8FF] ml-9 mt-1 rounded border border-[#AEB8CC] p-1">
              <button className="text-end py-0.5 pt-1 hover:bg-graybg rounded-lg flex items-center justify-end group transition-colors">Profile
